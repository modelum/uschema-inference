package es.um.uschema.neo4j.queries.inserts;
...
import es.um.uschema.neo4j.queries.inserts.cypher.Neo4jCypherMaker;
import es.um.uschema.neo4j.queries.inserts.cypher.Neo4jCypherStatementsStore;
import es.um.uschema.neo4j.queries.model.Neo4jNode;
import es.um.uschema.neo4j.queries.model.Neo4jReference;
import es.um.uschema.neo4j.queries.model.Neo4jReferenceToNewObject;
...
public class Neo4jInsertsExecutor implements AutoCloseable
{
	...
	public void createOrDropIndex(USchema uSchema, String property, String createOrDrop)
	{
		Neo4jCypherMaker cypherMaker = new Neo4jCypherMaker();
		try (Session session = driver.session())
		{
			Set<String> statements = cypherMaker.createIndexStatements(uSchema, property, createOrDrop);
